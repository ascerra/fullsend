...
Using GitHub's draft PR status to encapsulate the initial agent iterations (implement → review → fix cycles) before signaling that a PR is ready for human attention. Removing draft status serves as the notification trigger to humans. Draft mode is a middle ground between hiding all pre-submission work and flooding humans with work-in-progress notifications.
See [autonomy-spectrum.md](problems/autonomy-spectrum.md).
...
What changes are authorized, by whom, and at what tier. Intent answers "should this change exist?" before anyone asks "is this change correct?" In fullsend, intent is represented through forge issues, dedicated intent repos, RFCs, or tiered combinations. Without authorized intent, an agent has no basis for deciding what to work on.
See [intent-representation.md](problems/intent-representation.md) and [architecture.md](architecture.md).
