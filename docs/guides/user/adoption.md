...
**AGENTS.md and contributing guides** — Agents read these the same way human contributors do. If your contributing guide is outdated or missing, agents will make the same mistakes a new team member would. Update it to reflect how your team actually works. See [Configuring with AGENTS.md](customizing-with-agents-md.md) for guidance.
...
You can turn agents on or off individually — start with one, two, or all three depending on your adoption pace, and change at any time. Use `fullsend github setup ... --agents triage,prioritize,review` to scope which agents are active. See [Configuring GitHub](../getting-started/configuring-github.md) for details.

These agents have minimal impact on your team's workflow: they add comments that you can review and ignore if you disagree. This is a good way to get familiar with the platform and gauge the quality of its responses.
...
**Configure agent behavior** — environment variables offer quick behavioral tuning. For example, `REVIEW_FINDING_SEVERITY_THRESHOLD=medium` to adjust review sensitivity. See [configuring agent behavior](customizing-agents.md) for the full set of options.

For teams with specific needs, fullsend offers deeper configuration:

- **Skills** — extend agents with repo-specific capabilities, or replace built-in skills with your own. See [configuring with skills](customizing-with-skills.md).
...
Not every team will need all of these. Many teams find that a good AGENTS.md and a few environment variable tweaks are enough. See [configuring agents overview](customizing-agents.md) for a full overview, or [running agents locally](running-agents-locally.md) to test changes in your own environment.
...
Fullsend is also an agent runtime infrastructure: teams can build and deploy custom or derived agents alongside the defaults. See [Bring Your Own Agent](bring-your-own-agent.md) for the end-to-end workflow.
